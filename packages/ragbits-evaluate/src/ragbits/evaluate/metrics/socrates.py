...
from ragbits.evaluate.pipelines.socrates import SocratesResult


class SocratesOutcome(Metric[SocratesResult]):
    """
    Computes task success rate over SOCRATES tasks.
    Measures the fraction of tasks that were successfully solved.
    """

    @staticmethod
    async def compute(results: list[SocratesResult]) -> dict:
        """Compute task success rate.

        Returns:
            Dictionary with socrates_task_success_rate: fraction of successfully solved tasks.
        """
        success_count = sum(1 for r in results if r.task_success)
        success_rate = (success_count / len(results)) if results else 0.0

        return {"socrates_task_success_rate": float(success_rate)}


class SocratesTooling(Metric[SocratesResult]):
    """
    Tool utilization and performance metrics:
    - socrates_tool_trigger_rate: fraction of tasks where tools were used
    - socrates_avg_num_tool_calls: average number of tool calls per task
    - socrates_avg_tool_error_count: average number of tool errors per task
    ...
    async def compute(results: list[SocratesResult]) -> dict:
        ...
        averaged_freq: dict[str, float] = {
            f"socrates_tool_frequency_usage.{name}": (count / total_tasks) for name, count in aggregated_counts.items()
        }

        return {
            "socrates_tool_trigger_rate": float(tool_trigger_rate),
            "socrates_avg_num_tool_calls": avg_tool_calls,
            "socrates_avg_tool_error_count": avg_tool_errors,
            **averaged_freq,
        }


class SocratesEfficiency(Metric[SocratesResult]):
    """
    Efficiency and resource usage metrics:
    - socrates_avg_latency_ms: average response latency in milliseconds
    """
    ...
    async def compute(results: list[SocratesResult]) -> dict:
        ...
        return {
            "socrates_avg_latency_ms": avg_latency,
        }
