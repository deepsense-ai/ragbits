...
function CheckerDecisionChunkRenderer({ chunk }: ChunkRendererProps) {
  const data = chunk.chunk_data as { task_completed?: boolean; reason?: string; task_description?: string };
  const isCompleted = data.task_completed === true;
  return (
    <div className={`space-y-2 p-2 border rounded ${isCompleted ? "border-success bg-success/5" : "border-default bg-default-50"}`}>
      <div className="flex items-center gap-2">
        <Icon
          icon={isCompleted ? "heroicons:check-circle" : "heroicons:x-circle"}
          className={isCompleted ? "text-success" : "text-foreground-400"}
        />
        <span className="font-medium">{isCompleted ? "Task Completed" : "Task Not Completed"}</span>
      </div>
      {data.task_description && (
        <p className="text-xs text-foreground-500">
          Task: <span className="italic">{data.task_description}</span>
        </p>
      )}
      {data.reason && (
        <p className="text-sm text-foreground-600">{data.reason}</p>
      )}
    </div>
  );
}
...
// Aggregated text chunks indicator
...
const CHUNK_RENDERERS: Record<string, React.FC<ChunkRendererProps>> = {
  ...
  checker_decision: CheckerDecisionChunkRenderer,
};
...
export function ResponsesView({ scenarioName }: ResponsesViewProps) {
  ...
  // Filter chunks and aggregate consecutive text chunks
  const filteredChunks = useMemo(() => {
    if (!execution?.responseChunks) return [];
    let chunks = execution.responseChunks;
    if (filterType !== "all") {
      chunks = chunks.filter((c) => c.chunk_type === filterType);
    }

    // Aggregate consecutive text chunks
    const result: (ResponseChunk | { _aggregated: true; count: number; turn_index: number; chunk_index: number })[] = [];
    let textCount = 0;
