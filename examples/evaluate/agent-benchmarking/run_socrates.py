...
To use the planning-enabled variant, pass `socrates_planning_agent`. The script will
...
async def main(agent_path: str | None) -> None:
    ...
    )  # 9:14 includes 3 examples that didn't work for non-planning

    # Pipeline
    use_planning = "planning" in (agent_path or default_agent_path)
    log_file = "socrates_planning_examples.ndjson" if use_planning else "socrates_examples.ndjson"
