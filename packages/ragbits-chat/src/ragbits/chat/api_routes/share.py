"""FastAPI router that implements the conversation-sharing endpoints.

The routes operate on any :class:`HistoryPersistenceStrategy` that supports
ownership tracking (``get_conversation_owner`` / ``list_conversations``) and any
:class:`SharePersistenceStrategy`, so sharing does not depend on a specific
...
def build_share_router(  # noqa: PLR0915
    history: HistoryPersistenceStrategy,
    share: SharePersistenceStrategy,
    require_user: RequireUser,
) -> APIRouter:
    """Create an :class:`APIRouter` that exposes conversation-sharing endpoints.

    Args:
        history: History persistence with ownership/listing support.
        share: Share persistence strategy.
    ...
    async def list_conversations(user: User = authed_user) -> list[ConversationMeta]:
        ...
        shared_rows = await share.list_shared_with_me(recipient_identifiers(user))
    ...
    async def get_conversation(conversation_id: str, user: User = authed_user) -> ConversationDetail:
        ...
        if not is_owner:
            if not await share.can_access(conversation_id, recipient_identifiers(user)):
                raise HTTPException(status_code=404, detail="Conversation not found.")
            is_shared = True
            shared_by = owner
        ...
        if is_owner:
            raw_shares = await share.get_shares(conversation_id, user.user_id)
            shares = [
                ConversationShareResponse(
    ...
    async def dismiss_share(conversation_id: str, user: User = authed_user) -> Response:
        if not await share.hide_share(conversation_id, recipient_identifiers(user)):
