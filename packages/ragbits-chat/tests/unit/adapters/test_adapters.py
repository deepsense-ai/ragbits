...
class TestToolResultTextAdapter:
    ...
    async def test_tool_result_text_adapter_with_renderer() -> None:
        ...
        async def render_test_tool(tool_call: object) -> str:
            return f"Rendered: {tool_call.name}"  # type: ignore[attr-defined]
    ...
    async def test_tool_result_text_adapter_default_renderer() -> None:
        ...
        async def default_render(tool_call: object) -> str:
            return f"Default: {tool_call.name}"  # type: ignore[attr-defined]
...
class TestAdapterPipeline:
    ...
    async def test_pipeline_calls_lifecycle_hooks() -> None:
        ...
        class TrackingAdapter(BaseAdapter):
            ...
            async def on_stream_end(self, context: AdapterContext) -> AsyncGenerator[str, None]:  # noqa: PLR6301
    ...
    def test_pipeline_reset() -> None:
        ...
        adapter1._total = Usage()  # Create empty usage for test

        adapter2 = UsageAggregatorAdapter()
        adapter2._total = Usage()  # Create empty usage for test
