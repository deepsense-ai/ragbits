...
def test_perfect_matches(exact_match_results, matching_strategy):
...
def test_partial_matches(partial_match_results, matching_strategy):
    metric = DocumentSearchPrecisionRecallF1(matching_strategy)
    results = metric.compute(partial_match_results)
...
def test_no_matches(no_match_results, matching_strategy):
    metric = DocumentSearchPrecisionRecallF1(matching_strategy)
...
def test_rouge_threshold_behavior():
...
def test_mixed_results_with_multiple_queries(matching_strategy):
    results = [
...
def test_metric_set_with_different_strategies():
    config = {
...
def test_empty_retrieved_passages():
    results = [DocumentSearchResult(question="Q1", predicted_passages=[], reference_passages=["Important content"])]
...
def test_empty_reference_passages():
