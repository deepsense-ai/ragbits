...
async def print_planning_stats(log_path: Path, dataloader: _SupportsLoad) -> None:
    """Print aggregated planning-agent statistics from extended logs.

    Works across benchmarks if extended logs include the planner metadata.
    """
    print("\nPlanning Statistics:")
    decomposed_count = 0
    total_tasks = 0
            ...
            for line in f:
                ...
                if debug_log and debug_log != "[]":
                    ...
                    if debug_data and len(debug_data) > 0:
                        ...
                        planning_meta = metadata.get("planning", {})

                        if planning_meta.get("was_decomposed"):
                            decomposed_count += 1
                            total_tasks += int(planning_meta.get("num_tasks", 0))

                        if planning_meta.get("complexity_classification") == "SIMPLE":
                            simple_count += 1
                        elif planning_meta.get("complexity_classification") == "COMPLEX":
                            complex_count += 1
